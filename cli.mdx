...
`veri whoami` shows the active key + config file path. `veri doctor` runs a health check against the control plane.
...
| `veri jobs` | [Training](/cli/training) | Create, list, get, cancel, download, estimate training jobs |
| `veri deployments` | [Deployments](/cli/deployments) | Create, list, get, stop, chat, metrics, requests |
| `veri evals` | [Evals](/cli/evals) | Create, list, get, delete; `runs list/get/items` |
...
| `veri cost` | [Reference](/cli/reference) | `estimate`, `summary` |
...
| Flag | Effect |
| --- | --- |
| `-o`, `--format` | Output format: `text` (default), `json`, `jsonl`, `yaml` |
