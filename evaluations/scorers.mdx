...
Runs a standard task via lm-eval-harness (see the [benchmark catalog](/evaluations/benchmarks)). The task supplies its own rows, prompting, and scoring, so a benchmark eval has **no dataset**, and the benchmark scorer cannot be combined with other scorers.
...
| Field | Default | Meaning |
| --- | --- | --- |
| `task` | — | lm-eval-harness task id (`gsm8k`, `mmlu`, `ifeval`, ...) |
