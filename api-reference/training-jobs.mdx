...
<ParamField body="hyperparameters" type="object">
  ...
  <Expandable title="Agentic GRPO hyperparameters (method: grpo_agentic)">
    ...
    <Note>
      `grpo_agentic` requires H100-80GB GPUs and the `environments` reward source (mutually exclusive with `reward_function_id`).
    </Note>
  </Expandable>
...
<ParamField body="gpu" type="object" required>
  Explicit GPU configuration. Object with `gpu_type` (one of `A100-40GB`, `A100-80GB`, `H100-80GB`) and `gpu_count` (1–8).
</ParamField>
