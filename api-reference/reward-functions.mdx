...
keywords: ["reward functions", "api", "trl"]
---

<Note>
  There is no DELETE endpoint for reward functions in this release. Reward functions are content-addressed — uploading the same file twice returns the same id, so duplicates are not a problem in practice.
</Note>
...
<ParamField body="format" type="string" required>
  The reward function format. Supported value: `trl`.
</ParamField>
