...
1. **Invite the bot** to your Discord server. It posts a welcome message with a one-time claim link (the link expires after 7 days; @mention the bot to get a fresh one).
2. **Claim it** by opening the link while signed in to veri.studio. The confirm screen names the workspace and account it will bind to; the server becomes owned by that workspace, and its knowledge base, traces, and settings live there.
3. **Connect a source** right from chat: `@vchat connect github.com/your-org/your-repo`. The repository is indexed automatically and appears under Connected sources on the vchat page, with document counts and last-sync times.
4. Ask away: `@vchat how do I ...?`

Kicking the bot from the server unlinks it (your data stays in the workspace; re-inviting runs the claim flow again). You can also move the bot to another workspace you belong to from the vchat page (Transfer workspace in the settings menu): its sources, traces, and training history move with it, and the old workspace loses access.
...
Workspace admins can also **grade** any trace as Good or Bad directly in the dashboard (graded rows tint green or red). Grades are the curated feedback signal the improvement loop trains on, and grading is restricted to admins so casual reactions never steer your model.
...
2. **Training data synthesis.** Question-and-answer pairs are generated from the changed material, grounded in the retrieved passages that support them. Admin grades on traces (Good and Bad answers to the same kinds of questions) feed a preference-tuning pass, and that data goes through a stricter review step.
