...
Four steps from zero to answers:

1. **Invite the bot** to your Discord server. It posts a one-time claim link (valid 7 days; @mention the bot for a fresh one).
2. **Claim it**: open the link signed in to veri.studio. The confirm screen names the workspace and account the server will bind to.
3. **Connect a source** from chat: `@vchat connect github.com/your-org/your-repo`. It's indexed automatically and appears under Connected sources on veri.studio/vchat.
4. **Ask**: `@vchat how do I ...?` — answers come with citations into your repo.

Kicking the bot unlinks the server (your data stays in the workspace; re-inviting re-runs the claim flow). To move the bot to another workspace you belong to, use Transfer workspace in the vchat page's settings menu: sources, traces, and training history move with it, and the old workspace loses access.
...
The rest of this page explains how the system works end to end so you know exactly what happens to your data and where answers come from.
