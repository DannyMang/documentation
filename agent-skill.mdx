...
Prompt-suite evals are live under `/v1/evals/*` and the `veri evals` command group. A suite is
a yaml file (name + prompts, each with at least one check: `expect_contains`,
`expect_not_contains`, a `judge` rubric, or `tools_expected`); a run executes it against a
serving target and prints a per-prompt scorecard plus the diff vs. the previous run.

```bash
veri evals run suites/support.yaml --target dep_...        # or dep_...:<adapter>, or <model>@v6
veri evals runs --suite support | veri evals get evr_...   # history / one scorecard
veri models gate acme-bot --suite support                  # production promotes require a pass
```

Exit codes: 0 all prompts passed, 1 any failed, 2 API error. A `<model>@v6` target needs the
version live on a fleet (409 `version_not_servable` otherwise; target the deployment instead).
With a gate on, `veri models promote` answers 409 `eval_gate_failed` listing the failing prompt
ids. `veri run` configs remain `kind = "train" | "deploy"` only; evals run via `veri evals run`.

Docs: https://docs.veri.studio/evals
