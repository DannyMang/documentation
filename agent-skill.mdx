...
Every command and constraint below is verified against the live API and `veri-sdk` 0.2.34 (the latest release). When docs and this file disagree, trust `veri <cmd> --help` and [the docs](https://docs.veri.studio).
...
veri version                    # need sdk_version >= 0.2.34 (this skill's baseline); Python >= 3.10
...
If `sdk_version` is older than 0.2.34, upgrade before continuing — older CLIs are missing
commands this skill uses: `uv tool upgrade veri-sdk` (or `pipx upgrade veri-sdk` /
`pip install -U veri-sdk`). 0.2.34 is a hard floor for reward functions specifically:
in 0.2.33 and earlier `--reward` took the id of a stored reward resource, and in 0.2.34
it takes a path to a local `.py`. Passing a file path to an older CLI fails.
...
Not available: `/v1/evals*` routes are not registered (404). The `veri evals` command group
is not registered in the CLI either. Note that `veri run` still accepts a config with
`kind = "eval"` and will fail against the API — only `kind = "train"` and `kind = "deploy"`
work. `veri init` scaffolds a `configs/eval.toml` placeholder; ignore it. Don't propose eval runs.
