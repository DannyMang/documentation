...
## Harness variants: LangChain and Claude Agent SDK

The OpenAI SDK loop above is the default harness (and the verified training path). The same task runs on the other major agent stacks — each has its own subpage with the full harness and submit command:

- [LangChain](/demos/harness-url-extraction-agent/langchain) — `ChatOpenAI` + message types, trains over the same verified OpenAI route.
- [Claude Agent SDK / Anthropic](/demos/harness-url-extraction-agent/anthropic) — native `tool_use` blocks over `/v1/messages`; connectivity is verified, token capture is the open spike (train on the OpenAI route today).
