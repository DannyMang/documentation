...
The HF dataset has `arxiv_id` / `title` as inputs and `repositories` / `project_pages` lists as ground truth — but no `prompt` column, so we synthesize it. We keep rows with an **official** repository (cleanest ground truth). This is the whole "port": JSONL in, JSONL out, no environment spec:

```python prep_dataset.py
from datasets import load_dataset
import json

ds = load_dataset("Mithilss/neurips-2025-paperswithcode-artifacts", split="train")
