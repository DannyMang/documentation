...
Nothing to upload here: the file rides the submit itself (`--reward reward.py` in step 4).
...
<Warning>
  **Both routes are verified training paths** (token-id capture confirmed live on OpenAI and Anthropic `/v1/messages`). The Anthropic route supports native `tool_use` end to end: the `tools` array rides the policy's chat template, tool calls come back as `tool_use` blocks, and `tool_result` blocks round-trip — `agent.py` is a verified live path. Spans without token ids are rejected from training (fail-safe, never silent drift). The stock Claude Agent SDK runtime (`claude -p ...`, via `agent_claude.sh`) is also verified live; image and document content blocks are rejected with a 400 (no token-native path).
</Warning>
