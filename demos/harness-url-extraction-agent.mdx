...
<Tabs>
  ...
  <Tab title="OpenAI SDK">
    ...
    The **verified training path** today — token-id capture is confirmed on the OpenAI route.
  </Tab>
  <Tab title="LangChain">
...
<Warning>
  **For training-fidelity rollouts, the OpenAI route is the verified path today.** Token-id capture through the Anthropic route is unconfirmed: spans without token ids are rejected from training (fail-safe, never silent drift). Native tool_use also needs tool-calling flags on the vLLM side; if the model can't emit tool_use blocks it falls back to plain-text answers, which tells you the flags are missing. The Claude Agent SDK proper wraps the `claude-code` CLI (Node.js), which the Python rollout images don't ship yet.
</Warning>
...
For the verified training path (OpenAI route), swap the entrypoint: `--entrypoint "python agent_openai.py"` and drop `--harness-protocol` (openai is the default). LangChain: `--entrypoint "python agent_langchain.py"`.
