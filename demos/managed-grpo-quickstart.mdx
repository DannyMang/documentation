...
The shortest path to a trained model on Veri. [Managed GRPO](/training/managed) is **one decorator and one command**: define a reward function, decorate it with your config, and run `veri train`. Veri attaches the reward source to the job, connects the dataset, provisions the GPU, runs the loop, streams metrics, and captures the checkpoint.
...
The decorator is metadata only. `veri train` attaches the function's source as the job's reward, submits, and waits.
