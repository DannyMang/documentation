...
Both paths run on Veri-provisioned GPUs, stream logs and metrics, and capture the trained
checkpoint into your account. Managed jobs also save [intermediate checkpoints](/training/managed#checkpoints)
while training runs, so an interrupted job still leaves usable weights.
