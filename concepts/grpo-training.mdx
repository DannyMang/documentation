...
- **SF Compute** -- VM-based GPU instances.

Available GPU types include **A100 (80GB)**, **H100 (80GB)**, **L40S**, and others depending on the provider. GPU type and count must be specified explicitly in the `gpu` object on job creation. You can specify a `provider` or omit it to let Veri auto-select the cheapest available option.
