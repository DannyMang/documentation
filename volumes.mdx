...
keywords: ["volumes", "storage", "persistent storage", "veri volumes"]
---
...
| Tier | Rate |
| --- | --- |
| First 512 GiB per user (averaged across the month) | Free |
| Above 512 GiB | $0.085 per GiB-month |

The free tier is per **user**, not per volume — your usage is summed across every volume you own, averaged across the calendar month, and the first 512 GiB of that average is free.

Size is measured by **hourly snapshot**: the reconciler records each volume's `total_bytes` every hour, and at month-end the settlement job takes the per-volume mean (not the peak), sums to a user total, subtracts the 512 GiB free tier, and charges the remainder. Settlement runs once per day, but only the run on the 1st of the month UTC actually debits Stripe.
