...
Every command and constraint below is verified against the live API and `veri-sdk` 0.2.33 (the latest release). When docs and this file disagree, trust `veri <cmd> --help` and [the docs](https://docs.veri.studio).
...
veri version                    # need sdk_version >= 0.2.33 (this skill's baseline); Python >= 3.10
...
If `sdk_version` is older than 0.2.33, upgrade before continuing — older CLIs are missing
...
Not available: `/v1/evals*` routes are not registered (404). `veri evals ...` commands exist in the CLI but will fail against the API. Don't propose eval runs.
...
Docs: https://docs.veri.studio/volumes · https://docs.veri.studio/deployments/custom-models · https://docs.veri.studio/training/wandb · https://docs.veri.studio/training/huggingface

## REST API essentials

Base `https://api.veri.studio`, header `Authorization: Bearer vk_...`. Public spec: `GET /openapi.json`. Main resources: `/v1/training_jobs` (+ `/logs`, `/metrics`, `/cancel`, `/model`), `/v1/deployments` (+ `/chat/completions`, `/stop`, `/wake`), `/v1/datasets`, `/v1/reward_functions`, `/v1/volumes`, `/v1/models` (+ `/{id}/push-to-hf`, poll `/v1/hf_exports/{id}`), `/v1/settings/integrations/{wandb,huggingface}`, `/v1/billing/*`, `/v1/gpu/catalog`, `/v1/regions`. Errors: 400 invalid/gated, 401 bad key, 402 insufficient credit, 404 not found/not owned, 422 schema. Pagination: `limit` + `after` cursor, `has_more` in response.
