...
- **A standard benchmark** (rolling out) — GSM8K, MMLU, IFEval, and any other lm-eval-harness task. [Benchmarks](/evaluations/benchmarks) run on the community-standard harness with their own data, prompting, and scoring; you only pick a model, and scores are comparable with published numbers. Benchmarks may show as "coming soon" while this completes rollout.
...
3. The eval engine fans dataset samples out **concurrently** (up to 100 in flight) against your deployment's serving replicas. Benchmark runs execute in an isolated sandbox driven by lm-eval-harness.
4. Watch progress live; read the aggregate score and drill into per-sample items when it completes.

Dataset runs are resumable: progress is checkpointed per item, so a run interrupted by a platform deploy picks up where it left off. Benchmark runs restart from scratch if interrupted, which keeps the harness's sampling exactly as published.
...
- [Benchmarks](/evaluations/benchmarks) — the lm-eval-harness catalog and how sandboxed execution works
