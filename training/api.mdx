...
**Required:** `base_model`, `dataset_id`, `output_name`, `gpu`. **Required for `method=grpo` and `grpo_harness`:** `reward_source` (or `reward_sources`, or `environments`).

`dataset_id` also accepts [dataset stream](/training/dataset-streams) references: a stream's name or id with `@latest` or `@snap-N` (for example `acme-feedback@latest`). The job resolves and stores the pinned snapshot as `dataset_snapshot_id` in its response, so later appends never change what the job trains on.
...
- `404 Not Found` — `dataset_id` doesn't exist or belongs to another user
...
Supported `gpu_type` values: `A100-80GB`, `H100-80GB`, `L4-24GB`, `A10G-24GB`. AMD `MI300X-192GB` is coming soon. On AWS, `A100-80GB` and `H100-80GB` launch as 8-GPU nodes; `L4-24GB` and `A10G-24GB` are the single-GPU shapes. On [`vast`](/training/providers/vast), `A100-80GB` and `H100-80GB` rent as flexible 1-8 GPU chunks; on [`digitalocean`](/training/providers/digitalocean), `H100-80GB` comes in fixed 1x and 8x droplets. Run `veri gpu list` to see current availability and pricing for every shape.
