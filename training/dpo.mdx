...
| `use_liger` | `false` | Opt-in [liger-kernel](https://github.com/linkedin/Liger-Kernel) fused training kernels. Recommended for ≥4B models at batch sizes >1; measurably slower on small models at batch 1. Requires the package on the worker image (preinstalled on AMD providers). |
| `tunableop` | `false` | Opt-in PyTorch TunableOp GEMM autotuning (most useful on AMD MI300X). Tunes at runtime, so best for long runs with steady shapes. |
