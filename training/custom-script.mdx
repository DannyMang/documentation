...
<Note>
  Custom scripts run on NVIDIA GPUs only for now — the base-image catalog is CUDA-tier. A submit with an AMD `gpu_type` (e.g. `MI300X-192GB`) returns a 400. For AMD MI300X, use managed training (`grpo`, `sft_text`, `dpo`) on `provider="hotaisle"`; AMD custom-script support is planned.
</Note>
...
<Note>
  Multi-node gangs are not enabled yet — a `--num-nodes` of 2 or more currently returns a "coming soon" 400 while the feature finishes hardware validation. Single-node (`--num-nodes 1`, the default) is unaffected.
</Note>
