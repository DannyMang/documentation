...
Click a dataset on the dashboard's Datasets page to open a row viewer that lazily pages through the stored JSONL, so large datasets load incrementally as you scroll. Uploaded datasets are previewable immediately. Connected datasets (Hugging Face or volume) become previewable after their first training run creates the [snapshot](#snapshot-caching).
