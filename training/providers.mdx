...
description: "Choose where your training job runs: AWS (NVIDIA), Hot Aisle (AMD MI300X), or Vast.ai (NVIDIA marketplace spillover)."
keywords: ["providers", "AWS", "Hot Aisle", "AMD", "MI300X", "Vast.ai", "GPU", "cloud"]
---

Veri runs training jobs on three GPU providers. You pick one at job submit time with the `provider` field. Each provider offers different GPU types at different price points — AWS is the default, Hot Aisle is the AMD beachhead, and Vast.ai is the NVIDIA marketplace spillover.
