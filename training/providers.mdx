...
<Info>
  Hot Aisle VMs run ROCm, not CUDA. The training stack (torch, transformers, TRL) is installed from the ROCm wheel index at boot, with GEMMs routed through hipBLASLt. Unsloth, bitsandbytes, xformers, and vLLM are CUDA-only and are not available on this provider. Vanilla TRL training (GRPO, SFT, DPO) is fully supported, and liger-kernel fused kernels are preinstalled — enable them with `hyperparameters={"use_liger": true}`.
</Info>
