...
<Warning>
  Miles-format rewards are **NVIDIA-only**. The runner refuses jobs that pair `format=miles` with an AMD GPU.
</Warning>
...
Like datasets, reward functions are deduped by content hash — uploading the same file twice returns the same id.
...
<CardGroup cols={2}>
  ...
  <Card title="Submit a job" icon="play" href="/fine-tuning/api">
