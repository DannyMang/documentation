...
## `--dry-run` before billing

On `veri jobs create` (and `veri run` with `kind="train"`), `--dry-run` reports the estimated cost without submitting:
