...
| `import-traces <name>` | Import a window of a deployment's request traces as rows (`--score-evaluator` with `--passed`/`--failed`/`--min-score`/`--max-score` keeps only scored traces). See [Import request traces](/training/dataset-streams#import-request-traces). |
