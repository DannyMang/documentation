...
| `compare --type X --count N --hours H` | Side-by-side provider prices for a config. |

```bash
veri gpu list
veri gpu compare --type A100-80GB --count 2 --hours 4
```
...
| Command | Purpose |
| --- | --- |
| `estimate --gpu-type X --gpu-count N --hours H [--provider P]` | Quick cost calc. |
| `summary [--days N]` | Recent spend rollup. |

```bash
veri cost estimate --gpu-type A100-80GB --gpu-count 2 --hours 1
veri cost summary --days 30 --format json
```
...
| `burn-rate` | `$/day` plus estimated runway. |
| `transactions [--limit N]` | Recent charges. |

```bash
veri billing balance
veri billing burn-rate
