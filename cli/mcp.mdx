...
Beyond the local stdio server, `veri mcp serve-http` runs the same tool surface as a multi-tenant streamable HTTP service. This is the server behind Veri's hosted MCP endpoint, and you can also run it yourself (for example on an internal host shared by a team of agents).
