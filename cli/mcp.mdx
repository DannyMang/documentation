...
| `veri_list_evaluators` | List evaluators (LLM judge, code, human) with their latest version's method and output type. |
| `veri_get_experiment` | Get an experiment's status, per-evaluator results, verdict, and diff against its baseline. |
| `veri_get_model_gate` | Get a model lineage's promotion gate: its config, production's latest matching experiment, and judge-vs-human agreement warnings. |
...
| `veri_run_experiment` | Start an experiment on existing deployments (no new GPUs). Returns at once; poll `veri_get_experiment` for the result. |
...
Creating evaluators and running local code evaluators happen in the `veri` CLI
(`veri evaluators create`, `veri experiments run --local`): an experiment that pins a
local code evaluator waits in `awaiting_local` until those scores arrive.
