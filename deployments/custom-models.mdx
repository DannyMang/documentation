...
<Note>
  If you retrain the same model repeatedly, register the job into a versioned lineage instead of saving flat entries: `veri models register <job-id> --model acme-bot` makes it `acme-bot@v6` with `production`/`staging` aliases, audited promotion, and rollback. See [Model versions](/deployments/model-versions).
</Note>
