...
| `provider` | no | Auto-selects cheapest if omitted |
...
List your deployments. Filter by `status` (`queued` / `provisioning` / `serving` / `unhealthy` / `stopped` / `failed`).
...
Single deployment. Same shape as create response, with timestamps + cost populated as it runs.
...
Cost accrues during `serving` and `unhealthy` only.
...
These all hit the endpoints above.
...
<CardGroup cols={2}>
  ...
  <Card title="Cost & pricing" icon="dollar-sign" href="/deployments/cost">
    Pricing formula and idle warning.
  </Card>
