...
Deployments run on six providers. You pick one at create time with the `provider` field; it defaults to `aws`. There is no automatic cross-provider routing — you pick the provider explicitly, and capacity errors don't spill to another cloud.
